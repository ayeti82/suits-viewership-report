# 📊 Suits Viewership Analysis

This repository contains a basic data analysis report on the TV series **_Suits_**, using viewership data scraped from [Wikipedia](https://en.wikipedia.org/wiki/List_of_Suits_episodes).

The analysis includes:

- 📺 A brief overview of the show
- 📈 Viewership statistics and trends over 9 seasons
- 📉 Visualizations of viewership changes across episodes and seasons
- 🧼 Web scraping with `requests`, `BeautifulSoup`, and `pandas`

---

## 📂 Files

- `suits_viewership_analysis.ipynb`: Jupyter Notebook with full code, plots, and markdown commentary.
- `suits_viewership_report.html`: Rendered HTML version of the report (for offline viewing).
- `data/`: Contains cleaned `.csv` viewership data.

---

## 📌 How to View the Report

You can view the report in one of the following ways:

### ✅ 1. **Preview the `.ipynb` notebook** on GitHub
Just open [`suits_viewership_analysis.ipynb`](./suits_viewership_analysis.ipynb) in your browser to view code and output inline.

### 💾 2. **Download the rendered HTML report**
Download [`suits_viewership_report.html`](./suits_viewership_report.html) and open it in any web browser for a clean, code-free view.

---

## 🛠️ In order to reproduce it

- Python 3.7+
- Jupyter Notebook
- `pandas`, `requests`, `beautifulsoup4`, `matplotlib`, `seaborn`

```bash
pip install -r requirements.txt
